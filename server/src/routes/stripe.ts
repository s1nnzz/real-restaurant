...
router.post("/checkout/create-session", requireAuth, async (req, res) => {
	try {
		...
		const { items, useRewards } = req.body as CheckoutRequest;
		...
		if (menuItems.length !== items.length) {
			return res
				.status(400)
				.json({ message: "Some items are no longer available" });
		}
		...
		for (const cartItem of items) {
			...
			lineItems.push({
				price_data: {
					...
					unit_amount: Math.round(menuItem.price * 100), // Stripe expects pence
				},
				quantity: cartItem.quantity,
			});
		}

		// Apply rewards discount if requested
		let rewardsUsed = 0;
		let finalTotal = total;
		let couponId: string | undefined;

		if (useRewards) {
			const availableRewards = await Reward.getTotalAvailableRewards(
				userId
			);
			rewardsUsed = Math.min(availableRewards, total);
			finalTotal = total - rewardsUsed;

			// Create a one-time coupon for the rewards discount
			if (rewardsUsed > 0) {
				const coupon = await stripe.coupons.create({
					amount_off: Math.round(rewardsUsed * 100),
					currency: "gbp",
					duration: "once",
					name: `Rewards Discount (£${rewardsUsed.toFixed(2)})`,
				});
				couponId = coupon.id;
			}
		}

		// Create order in database (store final total after discount)
		const orderId = await Order.createOrder(userId, finalTotal);
		...
		const sessionParams: Stripe.Checkout.SessionCreateParams = {
			...
			metadata: {
				...
				rewardsUsed: rewardsUsed.toString(),
				originalTotal: total.toString(),
			},
		};

		// Add discount if rewards are being used
		if (couponId) {
...
// Note: express.raw() middleware is applied in index.ts for this route
		...
		case "checkout.session.completed": {
			...
			if (session.payment_status === "paid") {
				...
				const userId = session.metadata?.userId
					...
					: null;
				const rewardsUsed = session.metadata?.rewardsUsed
					? parseFloat(session.metadata.rewardsUsed)
					: 0;
				const originalTotal = session.metadata?.originalTotal
					? parseFloat(session.metadata.originalTotal)
					: 0;

				if (order) {
					await Order.updateOrderStatus(order.id, "paid");

					// Mark rewards as used if any were applied
					if (rewardsUsed > 0 && userId) {
						await Reward.useRewards(userId, rewardsUsed, order.id);
					}

					// Grant new rewards based on original order total (£2 per £10 spent)
					const rewardAmount =
						Reward.calculateRewardAmount(originalTotal);
